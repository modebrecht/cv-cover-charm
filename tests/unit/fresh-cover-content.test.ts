...
describe("Fresh cover content contract", () => {
  test("all templates 21-42 use a stable cover label instead of leaking legacy eyebrow copy", () => {
  ...
  test("all templates 21-42 keep application kicker and profession on separate logical lines", () => {
    for (const id of FRESH_TEMPLATE_IDS) {
      const template = id as TemplateId;
      const blocks = buildBlocks(template, candidateData, [], {}, slotsFor(template));
