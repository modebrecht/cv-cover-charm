...
function defsFor(template: TemplateId, data: CoverData): Def[] {
  ...
  if (template === "klassisch") {
    return [
      ...
      { id: "kicker", label: "Über-Titel", lines: ["Bewerbung"], style: s({ x: 20, y: 138, w: 170, size: 10, color: "accent", align: "center", uppercase: true, tracking: 0.5, font }) },
      { id: "beruf", label: "Titel (Beruf)", lines: data.beruf ? ["um eine Lehrstelle als", data.beruf] : [], style: s({ x: 25, y: 150, w: 160, size: 24, color: ink, align: "center", italic: true, lineHeight: 1.25, font }) },
