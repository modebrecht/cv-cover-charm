...
export function DossierHeaderFooterChrome({
  ...
  const headerMode = effectiveDossierHeaderMode(scope, pageIndex);
  ...
  const activeOptions = getDossierChromeOptions(scope);

  return (
    <div
      data-dossier-chrome={scope}
      data-dossier-header-mode={headerMode}
      data-dossier-footer-mode={activeOptions.footerMode}
    ...
    >
      ...
      {activeOptions.footerMode === "compact" ? (
        <div
      ...
      {activeOptions.footerMode === "details" ? (
        <div
        ...
        >
          {footerLabel && footerDetails.length ? (
            <>
              <div className="shrink-0 font-semibold">{footerLabel}</div>
              <div className="min-w-0 flex-1" style={{ overflowWrap: "anywhere" }}>
